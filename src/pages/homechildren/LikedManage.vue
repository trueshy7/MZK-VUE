<template>
  <div class="outer">
    <div class="top-input">
      ...
      <el-button @click="getLikesList" class="search-button" type="default">搜索</el-button>
    </div>
    <div class="table-show">
      <el-table :data="tableData" class="table-item-show">
        <el-table-column prop="likeId" label="key" class="table-item"/>
        <el-table-column prop="username" label="用户名" class="table-item"/>
        <el-table-column prop="title" label="歌曲名" class="table-item"/>
        <el-table-column prop="likeTime" label="点赞时间" class="table-item"/>
        <el-table-column fixed="right" label="操作" class="table-item">
          <template #default="scope">
            <el-button
                ...
                type="primary"
                size="small"
                @click="deleteRow(scope.row.likeId)"
            >
              删除
            </el-button>
          </template>
...
export default {
  ...
  methods: {
    getLikesList() {
      ...
      this.$axios.post("/likes/list", param).then((res) => {
    ...
    async deleteRow(id) {
      try {
        ...
        console.log(id);
        await this.$axios.delete("/likes/delete?id=" + id); // 等待删除完成
